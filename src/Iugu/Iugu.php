...
class Iugu
{
    ...
    public function postInvoice(InvoiceModel $invoice){
        ...
        return $this->send('POST', 'invoices', $data, 20);
    ...
    public function postCustomer(CustomerModel $customer){
        ...
        return $this->send('POST', 'customers', $data, 15);
     ...
     */
    private function send($method, $url, $data=array(), $timeoutInSeconds=null)
    {
        $data['headers']['Authorization'] 	= 'Basic '.$this->generateAuthorizationCode();

        if ($timeoutInSeconds) {
            $data['timeout'] = $timeoutInSeconds;
            $data['connect_timeout'] = $timeoutInSeconds;
        }
